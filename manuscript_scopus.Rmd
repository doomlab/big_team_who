...
***Institution and Geopolitical Region ***. We will use the affiliation ids and country to gather
information about the places of education and/or employment for authors.
Country will likely be binned into United Nation Region, Sub-Region,  or smaller clusters for
analyses.
...
ggplot(DF$career_time %>% 
  ...
  theme_classic() + 
  geom_point(aes(size = num_papers)) +
  facet_wrap(~SubjectArea) + 
  theme(legend.position = "bottom") + 
...
ggplot(DF$inst_time %>% 
       ...
       aes(Decade, `normalized`)) + 
  #geom_point() + 
  geom_violin() + 
  theme_classic() + 
  facet_wrap(~SubjectArea) + 
  ylab("Percent of Unique Affiliations") + 
  xlab("Decade")
```
...
pub_types <- bind_rows(
  ...
  filter(publication_type == "ar" | publication_type == "pp" | 
           publication_type == "dp")
...
# make a map on a continuous scale
ggplot(DF$geo_country) +
  geom_map(aes(map_id = affiliation_tag_country, fill = count_all), map = world_map) +
  geom_polygon(data = world_map, 
               aes(x = long, y = lat, group = group), 
               colour = 'black', fill = NA) + 
  theme_void() + 
...
ggsave("figure/continuous_country_all.png")
  
# maybe try binning
DF$geo_country$count_binned_all <- if_else(
  DF$geo_country$count_all >= 10000, "10,000+", 
  if_else(
...
ggsave("figure/binned_country_all.png", bin_country_all)
...
ggsave("figure/treemap_all.png", tree_all)
```

#### four panel plot with tree and country next to each other ####
...
small_dot + 
  ...
  ggtitle("10-49 Authors") + 
medium_dot + 
  theme(legend.position = "none") + 
  coord_cartesian(ylim = c(0,1), xlim = c(1980, 2025)) +
  ggtitle("50-99 Authors") +
large_dot + 
  theme(legend.position = "none") + 
  coord_cartesian(ylim = c(0,1), xlim = c(1980, 2025)) +
  ggtitle("100 + Authors") + 
