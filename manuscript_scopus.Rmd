...
geo_country <- import("py_outputs/geo_country.xlsx")
geo_region <- import("py_outputs/geo_region.xlsx")
```
...
geo_country$affiliation_tag_country <- toupper(geo_country$affiliation_tag_country)
geo_country <- subset(geo_country, affiliation_tag_country != "NONE")
geo_country$affiliation_tag_country[geo_country$affiliation_tag_country == "SCG"] <- "SRB"
...
# maybe try binning
geo_country$count_binned <- if_else(
  geo_country$count >= 10000, "10,000+", 
  if_else(
    geo_country$count < 10000 & geo_country$count >= 5000, "5,000-9,999",
    if_else(
      geo_country$count < 5000 & geo_country$count >= 1000, "1,000-4,999",
      if_else(
        geo_country$count < 1000 & geo_country$count >= 100, "100-999",
        "< 100"
      )
    )
  )
...
geo_country$count_binned <- factor(geo_country$count_binned, 
                                   levels = unique(geo_country$count_binned))

# map of binned data 
bin_country <- ggplot(geo_country) +
...
# tree map
geo_country$un_region_sub <- countrycode(
  sourcevar = geo_country$affiliation_tag_country,
  origin = 'iso3c', 
  destination = 'un.regionsub.name'
)

geo_country$un_region <- countrycode(
  sourcevar = geo_country$affiliation_tag_country,
  origin = 'iso3c', 
  destination = 'un.region.name'
)

geo_country$un_region[geo_country$affiliation_tag_country == "TWN"] <- "Asia"
geo_country$un_region_sub[geo_country$affiliation_tag_country == "TWN"] <- "Eastern Asia"

geo_country <- geo_country %>% 
  na.omit()

tree <- ggplot(geo_country, aes(area = count, fill = count_binned,
