...
for (year in unique(DF$small_first_gp$Year)){
  ...
  #### FIX WARNING ISSUE HERE#####
  temp <- suppressWarnings(chisq.test(as.matrix(temp_table)))
  mote_temp <- suppressWarnings(v.chi.sq(x2 = temp$statistic, n = sum(temp_table), 
...
ggplot(small_effect, aes(year,v)) + 
  geom_point() + 
  theme_classic() + 
  geom_pointrange(aes(ymin = low, ymax = high)) + 
  xlab("Year") + 
...
DF_decade$region <- factor(
  DF_decade$Country,
  levels = unique(DF_decade$Country),
  labels = 
)
...
ggplot(DF_decade, aes(who, Count, fill = Country)) + 
