...
research areas, age groups, and education levels should help to drive
science in the path of better materials, reliability, generalizability
and more robust sample sizes (when necessary) in a study
...
ggsave(filename = "figure/figure_1_pub_time.png")
```
...
ggsave(filename = "figure/figure_2_cloud.png")
```
...
```{r fig-author-year, include = TRUE, fig.cap="Number of authors included on big-team science papers per year by subject area. Given the large skew in the data, the left panel presents the median number of authors per manuscript, and the right panel presents the average number of authors per manuscript by year."}
avg_author_time <- 
  DF$author_time %>% 
  filter(SubjectArea != "Multidisciplinary") %>% 
...
ggsave(filename = "figure/figure_3_num_authors.png")
```
...
ggsave(filename = "figure/figure_4_career.png")
```
...
```{r fig-author-gpe, include = TRUE, fig.cap="A comparison of author affiliation geopolitical region across decades. F stands for first authors and O stands for other authors."}
...
```{r fig-effect-gpe, include = TRUE, fig.cap = "Effect size of the differences in representation for UN Regions for author affiliations in big-team science papers by year. Larger dots indicate more papers and authors represented in the calculation of effect size."}
...
to cancer and genetics research in medicine and oncology for medical and
