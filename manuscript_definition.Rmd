...
by the needs of the project and the researcher involved. While
...
answer increasingly complex questions, meet funder and university desire
...
For example, psychology has had an increase in BTS publications like the
...
NEED TO RUN ALL TEAMS SUBJECT AREA
...
ggplot(DF$career_time %>% 
  ...
  scale_color_manual(values = c("#CA0020", "#0571B0"), name = "Team Size") + # Red and Blue from RdBu ends
  guides(size = "none")
...
```{r career-results-teams, include = FALSE}
...
sample_size <- total_num <- 10000000
...
```{r career-results-bts, include = FALSE}
label <- c("Physical", "Social", "Health", "Life")
avg_b_values <- c(-0.692014, -0.365357, -0.451827, -0.508350)
avg_se_values <- c(0.002727, 0.005525, 0.001348, 0.001786)
sd_b_values <- c(-0.265538, -0.097108, -0.096730, -0.013958)
...
graph_data_bts$type <- factor(
  graph_data_bts$type, 
  levels = c("Coefficient", "SD Coefficient", "Effect Size")
)

graph_data_bts$analysis <- "Career"
```
