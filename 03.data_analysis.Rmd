...
# for (i in 1:length(first_probs_ed)){
#   first_ed <- first_probs_ed[[i]]
#   first_job <- first_probs_job[[i]]
#   last_ed <- last_probs_ed[[i]]
#   last_job <- last_probs_job[[i]]
#   
#   ed <- first_ed %>% full_join(last_ed, by = c("Var1" = "Var1")) %>% filter(Var1 != "<NA>")
#   ed$Freq.x[is.na(ed$Freq.x)] <- 0
#   ed$Freq.y[is.na(ed$Freq.y)] <- 0
#   ed_chi <- chisq.test(ed[ , c("Freq.x", "Freq.y")])
#   
#   ed_mote <- v.chi.sq(x2 = ed_chi$statistic, 
#                       n = sum(ed_chi$observed), 
#                       r = nrow(ed_chi$observed),
#                       c = ncol(ed_chi$observed))
#   
#   gpr_effects_ed$v[i] <- ed_mote$v
#   gpr_effects_ed$low_v[i] <- ed_mote$vlow
#   gpr_effects_ed$high_v[i] <- ed_mote$vhigh
#   
#   job <- first_job %>% full_join(last_job, by = c("Var1" = "Var1")) %>% filter(Var1 != "<NA>")
#   job$Freq.x[is.na(job$Freq.x)] <- 0
#   job$Freq.y[is.na(job$Freq.y)] <- 0
#   job_chi <- chisq.test(job[ , c("Freq.x", "Freq.y")])
#   
#   job_mote <- v.chi.sq(x2 = job_chi$statistic, 
#                       n = sum(job_chi$observed), 
#                       r = nrow(job_chi$observed),
#                       c = ncol(job_chi$observed))
#   
#   gpr_effects_job$v[i] <- job_mote$v
#   gpr_effects_job$low_v[i] <- job_mote$vlow
#   gpr_effects_job$high_v[i] <- job_mote$vhigh
# 
# }
```
